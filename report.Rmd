...
caption_4 <- 'Variance and cumulative variance for principal components calculated from prcomp().'
caption_5 <- 'Biplot of the first two principal components colored by subtype classification.'
caption_8 <- 'Heatmap of expressed genes by sample.'
```
...
bar_cols <-c('Variance Explained'='light blue')
  variance_tibble %>%
    mutate(principal_components = factor(principal_components,
                                       levels = paste0("PC", 1:nrow(variance_tibble)))) %>%
    ggplot() + 
